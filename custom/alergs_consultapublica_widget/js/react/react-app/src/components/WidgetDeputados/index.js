...
const WidgetDeputados = ({
    ...
    const renderHtml = () => {
        ...
        if(data) {
            shuffle(data).map((item, index) => {
                ...
                ><img src="${item.fotoGrandeDeputado}" /></div>`
                ...
                class="item-inner--card"
                data-${optionValue}="${item[optionValue]}"
                ><img src="${item.fotoGrandeDeputado}" /></div></div>`    
    ...
    return (
        <Row className={`widget-deputados ${selectedOption && Object.keys(selectedOption).length !== 0 ? '--selected' : ''}`}>
                    ...
                    <Row>
                        ...
                        <Col xs={7}>
                            ...
                            <TextCard>
                                ...
                                <strong>E-mail:</strong> {selectedOption.emailDeputado}<br/>
          ...
          <Col className="p-5" xs={12} md={6}>
            {tpl && (<Carousel afterChange={(e) => { setCarousel(e) }} dots={false} arrows prevArrow={carousel > 0 && (<LeftOutlined />)} nextArrow={<RightOutlined />}>
                {ReactHtmlParser(tpl)}
            </Carousel>)}
          </Col>
        </Row>
